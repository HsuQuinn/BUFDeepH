...
M_trans = [3 -4 0; 4 7 0; 0 0 1]        # To do : Read M
N = 4                                   # number of g-vectors in |p+g> , equal to 2(N+1)^2
max_iter = 800
num_band = 2400                         # bands num in LC
fermi_level_shift = 0.0                 # fermi level shift for deep band
num_points = 20
mid_z_coord = 18.0                      # z coordinate of the middle plane, a lazy input for developer.
y_down = -5                             # y axis down limit
...
pp_points = vcat(path_KG, path_GM[2:end]) 
p_points = pp_points
...
outfile = joinpath(parsed_args["output_dir"], "data.txt")
